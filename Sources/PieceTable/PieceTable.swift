...
extension PieceTable {
    /// Returns the internal index (piece + content offset) corresponding to a linear character offset.
    ...
    /// Total number of characters represented in the piece table.
...
extension PieceTable {
    ...
    public func debugString(in range: NSRange?) -> String {
        ...
        for piece in pieces {
            ...
            for i in start..<end {
                // Insert caret *before* this character if needed
                ...
                let char = String(utf16CodeUnits: [sourceArray[i]], count: 1)
                result.append(char)
    ...
    public func string(in range: NSRange) -> String {
           ...
           for piece in pieces {
               ...
               if localStart < localEnd {
                   ...
                   result.append(contentsOf: String.UnicodeScalarView(slice.compactMap { UnicodeScalar($0) }))
